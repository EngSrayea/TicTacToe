...
//  TicTacToe
//
//  Created by Eng. Norii on 5/11/20.
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        ZStack {
            Image("tt3").resizable().frame(width:480, height: 1000).scaledToFill()
            VStack(spacing: 10){
                Text(winner).foregroundColor(.yellow).foregroundColor(.white).font(Font.custom("PWChalk", size: 60)).offset(y: -50)
                
                Text("\(crrntPlayer) Turn").foregroundColor(.white).font(Font.custom("PWChalk", size: 60)).offset(y: -50)
                    ...
                    HStack(spacing: 10){
                        ForEach(0 ..< 3){ c in
                            Button(action: {
                                ...
                                Text(self.fields[r][c].text).foregroundColor(.yellow).font(Font.custom("PWChalk", size: 70))
                                    .frame(width: 100, height: 100).background(Color.white).opacity(0.4)
                            }
                        }
                    }
                }
                ...
                if winner != "" {
                    Button(action: {
                        ...
                        Text("RESTART GAME?").foregroundColor(.white).font(Font.custom("PWChalk", size: 35)).offset(y: 50)
                    }
                }
